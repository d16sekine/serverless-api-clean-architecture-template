...
describe('hello handler', () => {
  const mockContext: Context = {
    ...
    logStreamName: '2023/01/01/[1]abcdefghijklmnopqrstuvwxyz',
  ...
  it('should return hello message with correct structure', async () => {
    ...
    expect(body).toHaveProperty('message', 'Hello from Serverless TypeScript Lambda!');
    expect(body).toHaveProperty('timestamp');
    expect(body).toHaveProperty('requestId', mockContext.requestId);
  ...
  it('should handle errors gracefully', async () => {
    ...
    const result = await handler(mockEvent as APIGatewayProxyEvent, invalidContext);
